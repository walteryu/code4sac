...
4. Resulting dataset = 5k rows x 27 columns; test dataset may be extracted from full dataset if prediction or model validation is necessary  
...
cat("\n")
cat("*** output summary; train_col and zipcode ***")
cat("\n")

# EDA; train_col$ZIP:
# summary(train_col$ZIP)

cat("\n")
cat("*** output summary; train_col and district***")
cat("\n")

# EDA; train_col$CouncilDistrictNumber:
# summary(train_col$CouncilDistrictNumber)
...
Note: Section below pertains only to continous variables; as a result, preserve code for later use but not running it here since dataset contains all categorical variables with exception of GIS lat/long data.  
...
3. Stacked bar chart shows that all variables follow noyes variables, indicating their importance and are worth additional review; this observation confirms finding that variables are significant  
...
#   main='ZIP',
#   col='purple',
#   xlab='Continuous Variable - Zipcode',
#   breaks=30
# )
...
2. Contingency tables show distributions of no/yes values for each significant factor and breaks them down by level  
...
3. ISLR p.286; logistic regression -XX  
