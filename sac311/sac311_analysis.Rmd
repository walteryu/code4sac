...
311 service call dataset from City of Sacramento: https://data.cityofsacramento.org/datasets/08794a6695b3483f889e9bef122517e9_0  
  
The data is available in geospatial, tabular or API format; the tabular format is used for this analysis to identify important neighorhoods and trends which are relevant to developing the application.  
  
Notes:  
1. All sources are cited accordingly.  
2. Source dataset is ~400MB, so not included in this repository.  
...
Steps:  
1. Import data, evaluate dimensions; note 2 distinct datasets  
2. Merge both datasets; address discrepancie attribute  
3. Evaluate categorical levels for country; consolidate levels  
4. Evaluate differences and separate into groups  
5. Verify results with summary statics; dim, str and summary functions  

Observations:  
1. Import dataset; includes header so note so in read.table  
2. Dataset does not appear to have NA or null values; however, check with functions during EDA just in case  
3. Import both datasets and calculate summary statistics  
4. Use str() function to evaluate factors/variables  
...
# EDA; train_data$ZIP:
summary(train_data$ZIP)

# EDA; train_data$CouncilDistrictNumber:
summary(train_data$CouncilDistrictNumber)
...
Observations:  
1. Create histograms for significant factors based on p-values -XX  
2. Create stacked bar charts for significant factors based on p-values -XX  
3. Histogram shows right-tail for mt; most values near median for zq; fairly equal distribution for zf; left-tail distribution for ihj -XX  
4. Stacked bar chart shows that all variables follow noyes variables, indicating their importance and are worth additional review; this observation confirms finding that variables are significant -XX  
5. In general, charts offered insights for additional analysis; bin width and values chosen to optimize data visualization -XX  
...
cat("\n")
cat("*** create histogram for continous variables ***")
cat("\n")

# Create historgram for continous variables to evaluate distribution:
hist(
  train_data$CouncilDistrictNumber,
  main='CouncilDistrictNumber',
  col='purple',
  xlab='continuous variable - council district number',
  breaks=30
)

cat("\n")
cat("*** create histogram, stacked bar chart ***")
cat("\n")
...
qplot_mt = (qplot(
  train_data$mt,
  binwidth = 0.1,
  fill=train_data$noyes,
  main='outcome variable - noyes')
  + scale_fill_manual(values=c('purple', 'red'))
  + theme(legend.position='top')
  + xlab('significant factor - mt')
)
qplot_mt
...
References:  
1. HW9, 10, 11 code; contingency table usage -XX  
2. Lecture 1, 3, 8; contingency tables -XX  
3. Lecture 1, 2, 3; data distribution -XX  
4. ISLR p.67; data distribution -XX  
5. ISLR p.76; data distribution -XX  

Steps:  
1. Evaluate data distribution with contingency tables -XX  
2. Evaluate distribution, then adjust for proportions -XX  
3. Create contingency tables for select attributes -XX  
4. Compare/evaluate results -XX  
5. Document observations below -XX  

Observations:  
1. Create contingency tables for significant factors based on p-values -XX  
2. Contingency tables show distributions of no/yes values for each significant factor and breaks them down by level -XX  
3. Contingency tables show different distributions for most factors; however, levels for "at" generally indicate "no" value for each level/gene/characteristic -XX  
4. In general, contingency tables are good for visualizing distributions by level -XX  
5. They are a good method of EDA for categorial variables -XX  
...
cat("\n")
cat("*** create contingency table; cle ***")
cat("\n")

# Create contingency table, cle:
table(
  train_data$noyes,
  train_data$cle
)
...
cat("\n")
cat("*** create contingency table; kbc ***")
cat("\n")
...
cat("\n")
cat("*** create contingency table;gb ***")
cat("\n")
...
cat("\n")
cat("*** create contingency table; xxp ***")
cat("\n")
...
cat("\n")
cat("*** create contingency table; rao ***")
cat("\n")
...
cat("\n")
cat("*** create contingency table; at ***")
cat("\n")
...
cat("\n")
cat("*** create contingency table; fq ***")
cat("\n")
...
References:  
1. Lecture 5, 6, 11 notes; PCA model/plots-XX  
2. Midterm and HW6 code; PCA model/plot usage -XX  
3. ISLR p.230; PCA components -XX  
4. ISLR p.233; PCA components -XX  
5. -XX  

Steps:  
1. Verify/prepare data for PCA plots; use dummy variables to setup factors -XX  
2. Scale/fit data for PCA plots; plot for top contributors to variance -XX  
3. Evaluate top contributors to variance -XX  
4. Plot variance by first several PCA -XX  
5. Plot/evaluate PCA by ptj and vuu -XX  

Observations:  
1. Yes, scaling is necessary prior to PCA plot -XX  
2. Use dummy variables to address muiple variable levels prior to PCA plot -XX  
3. Plot PCA components; ptj and vuu are largest variance contributors; also, both are continuous variables without levels -XX  
4. PCA plot for noyes and ptj visualizes variance contributions -XX  
5. PCA plot for noyes and vuu visualizes variance contributions -XX  

*** Plot variance for top 5 PCA ***

*** Evaluate largest loading for first PC ***
      ptj
0.5793067

*** Evaluate largest loading for second PC ***
      vuu
0.6895659

*** Evaluate largest loading for third PC ***
       zq
0.7718774

*** Evaluate largest loading for fourth PC ***
       id
0.9438584

*** Evaluate largest loading for fifth PC ***
       mt
0.7508143

*** summary() function output for train_data$ptj ***
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
  1.315   1.659   1.749   1.792   1.879   4.720

*** summary() function output for train_data$vuu ***
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
  0.000   2.830   3.320   3.024   3.610   3.740
...
# Evaluate largest loading for first and second PC:
biplot(
  prcomp_plot,
  pc.biplot=TRUE,
...
cat("\n")
cat("*** PCA Plot for noyes and ptj ***")
cat("\n")

# PCA plot:
plot(
  prcomp_plot$x[,1:2],
  main='PCA Plot: noyes and ptj (yes = red, no = purple)',
  col=c(ifelse(train_data$noyes == 'yes', 'red', 'purple')),
  cex=1.0,
  pch=ifelse(train_data$ptj == '>=1.75', '*', '+')
)

cat("\n")
cat("*** PCA Plot for noyes and vuu ***")
cat("\n")

# PCA plot:
plot(
  prcomp_plot$x[,1:2],
  main='PCA Plot: noyes and vuu (yes = red, no = purple)',
  col=c(ifelse(train_data$noyes == 'yes', 'red', 'purple')),
  cex=1.0,
  pch=ifelse(train_data$vuu == '>=3', '*', '+')
)

```

```{r final-p1.4A, results="hide"}

# reduce to smaller sample size to reduce runtime:
train_scaled_reduced = train_scaled[1:3500,]
...
References:  
1. Lecture 1, 3 notes; chi-squared test -XX  
...
Steps:  
1. Calculate chi-squared test for categorical variables -XX  
4. Create plots for significant factors based on p-values -XX  
4. Create plots for significant factors based on p-values -XX  
2. Evaluate/analyze results -XX  
3. Document observations below -XX  
4. -XX  
5. -XX  

Observations:  
1. Chi-squared test shows low p-values for categorial variables; t-test for continous variables -XX  
2. As a result, this observations confirms previous conjecture that variables should be kept as factors since they are significant predictors -XX  
3. Per lecture notes, chi-squared and t-test are a good method for evaluating significance -XX  
4. -XX  
5. -XX  

Observations:  
1. Create plots for significant factors based on p-values -XX  
2. Chi-squared and t-tests confirmed that all variables are significant per logistic regression results; specifically, all varaibles have p-value < 0.05 -XX  
3. As a result, these variables are significant and should be used for analysis -XX  
4. -XX  
5. -XX  
...
cat("\n")
cat("*** T-Test for continous variables, mt: ***")
cat("\n")
...
# Chi-Squared test for continuous variables, mt:
t.test(
  as.numeric(train_data$noyes),
  train_data$mt
)

cat("\n")
cat("*** T-Test for continous variables, zq: ***")
cat("\n")
...
# Chi-Squared test for continuous variables, zq:
t.test(
  as.numeric(train_data$noyes),
  train_data$zq
)
...
cat("\n")
cat("*** T-Test for continous variables, zf: ***")
cat("\n")
...
# Chi-Squared test for continuous variables, zf:
t.test(
  as.numeric(train_data$noyes),
  train_data$zf
)
...
cat("\n")
cat("*** T-Test for continous variables, ihj: ***")
cat("\n")
...
# Chi-Squared test for continuous variables, ihj:
t.test(
  as.numeric(train_data$noyes),
  train_data$ihj
)
...
cat("\n")
cat("*** Chi-Squared test for categorical variables, cle: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, cle:
chisq.test(
  table(train_data$noyes, train_data$cle)
)

cat("\n")
cat("*** Chi-Squared test for categorical variables, kbc: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, kbc:
chisq.test(
  table(train_data$noyes, train_data$kbc)
)

cat("\n")
cat("*** Chi-Squared test for categorical variables, gb: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, gb:
chisq.test(
  table(train_data$noyes, train_data$gb)
)

cat("\n")
cat("*** Chi-Squared test for categorical variables, xxp: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, xxp:
chisq.test(
  table(train_data$noyes, train_data$xxp)
)

cat("\n")
cat("*** Chi-Squared test for categorical variables, rao: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, at:
chisq.test(
  table(train_data$noyes, train_data$at)
)

cat("\n")
cat("*** Chi-Squared test for categorical variables, at: ***")
cat("\n")
...
cat("\n")
cat("*** Chi-Squared test for categorical variables, fq: ***")
cat("\n")
...
# Chi-Squared test for categorical variables, fq:
chisq.test(
  table(train_data$noyes, train_data$fq)
)
...
Observations:  
1. Yes, there appears to be association between the predictors and outcome based on tests completed within this section -XX  
2. Variables within this problem all demonstrated significance -XX  
3. Most variables have low p-values, indicating significance as a predictor -XX  
4. Both chi-squared and logistic regression tests show low p-values indicating significance for variables as predictors -XX  
5. As a result, these variables are associated with the outcome variable -XX  
6. Use LR results to develop charts in 1.2, 1.3 and 1.4-XX  
7. Use LR to fit categorical variables for additional analysis -XX  
...
cat("\n")
cat("*** logistic regression model fit, all variables: ***")
cat("\n")
...
# categorial = cle, kbc, gb, xxp, rao, at, fq
# logistic regression model fit, all variables:
glm_train = glm(
  noyes ~ .,
  data=train_data,
  family=binomial
...
cat("\n")
cat("*** logistic regression model fit, cle: ***")
cat("\n")
...
# categorial = cle, kbc, gb, xxp, rao, at, fq
# logistic regression model fit, cle:
glm_train = glm(
  noyes ~ cle,
  data=train_data,
  family=binomial
...
cat("\n")
cat("*** logistic regression model fit, kbc: ***")
cat("\n")
...
# categorial = cle, kbc, gb, xxp, rao, at, fq
# logistic regression model fit, kbc:
glm_train = glm(
  noyes ~ kbc,
  data=train_data,
  family=binomial
...
cat("\n")
cat("*** logistic regression model fit, gb: ***")
cat("\n")
...
# categorial = cle, kbc, gb, xxp, rao, at, fq
# logistic regression model fit, gb:
glm_train = glm(
  noyes ~ gb,
  data=train_data,
  family=binomial
...
cat("\n")
cat("*** logistic regression model fit, xxp: ***")
cat("\n")
...
# categorial = cle, kbc, gb, xxp, rao, at, fq
# logistic regression model fit, xxp:
glm_train = glm(
  noyes ~ xxp,
  data=train_data,
  family=binomial
...
cat("\n")
cat("*** logistic regression model fit, rao: ***")
cat("\n")
...
cat("\n")
cat("*** logistic regression model fit, at: ***")
cat("\n")
...
cat("\n")
cat("*** logistic regression model fit, fq: ***")
cat("\n")
...
4. Use pairs plot to verify variables for additional analysis -XX  
5. Pairs plot output is shown in extra problem, Part 2A -XX  
...
References:  
1. Midterm-P2 and HW5; best subset variable selection -XX  
2. ISLR p.244; best subset selection -XX  
3. ISLR p.247; best subset selection -XX  
4. Lecture 9, 12, 13 notes; best subset -XX  
5. -XX  

Steps:  
1. Use variable selection to identify best predictors -XX  
2. Use forward, backward and seq replacement methods -XX  
3. Create metrics plot and individual plot for each method -XX  
4. Evaluate/compare plot results -XX  
5. Document observations below -XX  

Observations:  
1. Each method returns the same variables and order of selection -XX  
2. Rank order of selection: 1) rao, 2) gb, 3) xxp, 4) at, 5) fq -XX  
3. Results confirm previous findings/analysis/conjecture -XX  
4. Significant variables will be useful for fitting other models -XX  
5. Best subset plot visualizes results for evaluation -XX  
