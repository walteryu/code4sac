...
To-Do List:
1. Spatial Analysis - Identify trends from location data using R spatial analysis packages; specifically, are there any specific issues for a neighborhood?  
2. Time series analysis - Identify trends in service calls over time; identify temporal patterns of calls.  
...
# create histogram, stacked bar chart:
...
References:  
Spatial plot: https://www.rspatial.org/analysis/4-interpolation.html#temperature-in-california  
...
```{r p1.3A, fig.height = 6, fig.width = 12, fig.align = "center"}
...
```{r p1.3B, fig.height = 8, fig.width = 8, fig.align = "center"}
...
# plot(county , col='light blue', border='gray')
# plot(county , col='light blue')

# plot points:
plot(county)
points(train_col[,c('Longitude', 'Latitude')], col='blue', pch=20)
# points(train_col[,c('XCoord', 'YCoord', 'CategoryLevel1')], cex=.5)
# points(train_col, cex=.5)
...
2. Plot neighborhood with service call categories for comparison  
3. Document observations below   

Observations:  
1. Create contingency tables for significant factors related to the project  
2. Contingency tables show distributions of variables and breaks them down by level; certain neighborhoods show association with particular categories  
3. These associations will help identify potential issues by neighborhood  
...
cat("\n")
cat("*** t-test for continuous variables ***")
cat("\n")
...
# LR for neighborhood variable; remaining model fits are below:
glm_train = glm(
  Neighborhood~.,
  data=train_col,
  family=binomial
